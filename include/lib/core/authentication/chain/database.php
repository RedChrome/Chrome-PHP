 ...
 * @subpackage Chrome.Authentication
 */
class Chrome_Authentication_Create_Resource_Database implements
	Chrome_Authentication_Create_Resource_Database_Interface
{
	protected $_identity = '';
	protected $_credential = '';
...
class Chrome_Model_Authentication_Database extends Chrome_Model_DB_Abstract
{
	...
	public function updateTimeById( $id )
	{
		...
		$this->_dbInterfaceInstance->update( $this->_options['dbTable'] )->set( array( $this->_options['dbTime'] =>
				CHROME_TIME ) )->where( 'id = "' . $id . '"' )->limit( 0, 1 )->execute();
