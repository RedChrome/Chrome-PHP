    ...
    private function _loadFile()
    {
        ...
        if(!_isFile(BASEDIR . self::CHROME_LANGUAGE_INCLUDE_DIR . $this->_language . self::CHROME_LANGUAGE_GENERAL_NAMESPACE . self::CHROME_LANUGAGE_FILE_EXTENSION))
        {
            throw new Chrome_Exception(
                'Cannot load file ' . BASEDIR . self::CHROME_LANGUAGE_INCLUDE_DIR . $this->_language . self::CHROME_LANGUAGE_GENERAL_NAMESPACE .
                self::CHROME_LANUGAGE_FILE_EXTENSION . '! File does not exist in Chrome_Language::_loadFile()!'
            );
        }
        ...
        {
            self::$_array[$this->_language][self::CHROME_LANGUAGE_GENERAL_NAMESPACE] = parse_ini_file(
                BASEDIR . self::CHROME_LANGUAGE_INCLUDE_DIR . $this->_language . '_general' .
                self::CHROME_LANUGAGE_FILE_EXTENSION, false
            );
        }
    }

    public function merge(Chrome_Language_Interface $language)
    {
        self::$_array[$this->_language][$this->_file] = array_merge(self::$_array[$this->_language][$this->_file], $language->getAll());
    }
