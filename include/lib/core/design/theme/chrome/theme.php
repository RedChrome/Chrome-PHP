 ...
 * @version    $Id: 0.1 beta <!-- phpDesigner :: Timestamp [27.03.2013 15:45:26] --> $
 */
...
class Chrome_Design_Theme_Chrome implements Chrome_Design_Theme_Interface
{
    ...
    public function initDesign(Chrome_Design_Interface $design) {
        ...
        $compositions = array('head' => $head, 'preBodyIn' => $preBodyIn, 'leftBox' => $leftBox, 'rightBox' => $rightBox,
            'body' => $body, 'footer' => $footer, 'postBody' => $postBody);

        // apply loaders
        foreach($compositions as $key => $composition) {

            $composition->setPosition($key);

            $loader = new Chrome_Design_Loader_Static($design->getApplicationContext());
